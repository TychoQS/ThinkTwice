...
locales.forEach(file => {
  ...
  // Clean up if it was appended at the end of the file by some broken multiple replace tool
  
  // Now add correctly at the end
  if (!content.match(/crisis:\s*\{\s*title:\s*['"](Krisen|Набор|危机|위기|危機)/)) {
      // Find the last '};' or '}' and inject it before.
      const lastBraceIndex = content.lastIndexOf('}');
      content = content.substring(0, lastBraceIndex) + t + '\n' + content.substring(lastBraceIndex);
  }
